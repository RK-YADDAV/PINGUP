      ...
      <div className='max-w-6xl mx-auto p-6'>
        ...
        <div className='mb-8 flex flex-wrap gap-6'>
          {dataArray.map((item, index) => {
            ...
            return (
              <div key={index} className='flex flex-col items-center justify-center gap-2 border h-24 w-40 border-gray-200 bg-white shadow rounded-md'>
        ...
        <div className='inline-flex flex-wrap items-center border border-gray-200 rounded-md p-1 bg-white shadow-sm'>
          {dataArray.map((tab) =>(
            <button onClick={()=> setCurrentTab(tab.label)} key={tab.label} className={` cursor-pointer flex-1 items-center px-3 py-1 text-sm rounded-md transition-colors 
                ...
                <div className='flex max-sm:flex-col gap-2 mt-4'>
                  ...
                  }
                  {
                    currentTab==='connections' && (
                      <button onClick={() => navigate(`/messages/${user._id}`)} className='w-full p-2 text-sm rounded bg-slate-100 hover:bg-slate-200 text-slate-200 active:scale-95 transition 
...
//               <div
//                 key={index}
//                 className='flex flex-col items-center justify-center gap-2 border h-24 w-40 border-gray-200 bg-white shadow rounded-md'
//               >
//                 <Icon className='text-indigo-500' size={24} />
//                 <b className='text-lg'>{item.value.length}</b>
//                 <p className='text-slate-600'>{item.label}</p>
